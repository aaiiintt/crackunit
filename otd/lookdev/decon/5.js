...
// that actually appears in the day's text, exploded into its frames:
...
function setup() {
  ...
  const words = OTD.dayWords();       // every library word that appears in this day's text
  if (!words.length) return;
  const w = words[(window.SEED - 1) % words.length];
  const it = OTD.stickerByMood(w.word, 120, /pixel|glitch|typo|type/) || w.item;
  stk = { word: w.word, sentence: w.sentence, it, img: OTD.loadSticker(it) };
}
function draw() {
  if (!stk) return OTD.skip("no library word appears in the day\u2019s text");
  ...
  for (const l of OTD.wrap(stk.sentence, 860)) { fill(255); rect(56, ty - 48, textWidth(l) + 16, 64); fill(0); text(l, 64, ty); ty += 64; }
