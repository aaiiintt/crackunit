...
function draw() {
  ...
  const raw = OTD.allFrames();
  if (raw.length < 6) return OTD.skip("no video frames that day");
  ...
  const pi = treated.filter((f) => f.id === IDS[0]).map((f) => f.img);
  ...
  const say = OTD.videos().map((p) => random(p.sentences || [""])).slice(0, 2);
  ...
  fill(0); OTD.label(`crackunit.com · ${OTD.dateWords()} · ${raw.length} frames`, 60, 1350 - 48, 14, "#fff");
  OTD.done();
}
